...
class TestAccessibilityMetrics:
    ...
    def test_road_density_calculation(self):
        """道路密度計算テスト"""
        density = AccessibilityMetrics.calculate_road_density(self.simple_graph, area_ha=1.0)
    ...
    def test_average_shortest_path(self):
        """平均最短距離計算テスト"""
        avg_path = AccessibilityMetrics.calculate_average_shortest_path(self.simple_graph)
        # 線形グラフの平均最短距離は計算可能
        assert avg_path > 0
...
class TestSpaceSyntaxMetrics:
    ...
    def test_calculate_all_metrics(self):
        ...
        # 指標の存在確認
        required_metrics = [
            'mu_index', 'alpha_index', 'beta_index', 'gamma_index',
            'avg_shortest_path', 'road_density', 'intersection_density',
            'avg_circuity'
        ]
